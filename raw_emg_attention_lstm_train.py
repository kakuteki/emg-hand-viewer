"""
Raw EMG Attention-LSTM モデルでの学習・評価
MelSpectrogram版と同条件で比較するため、window_size=64で学習
"""
...
# =====================
# ハイパーパラメータ（MelSpectrogram版と同じ）
# =====================
WINDOW_SIZE = 64  # MelSpec版と同じ
BATCH_SIZE = 1024
HIDDEN_SIZE = 256
NUM_LAYERS = 2
FC_SIZE = 64
...
print("="*60)
print("Raw EMG Attention-LSTM Model Training")
print("(Same conditions as MelSpectrogram version)")
print("="*60)
...
# Attention-LSTMモデル定義
# =====================
class RawEMGAttentionLSTM(nn.Module):
    def __init__(self, input_size=16, hidden_size=256, num_layers=2, fc_size=64,
        ...
        super(RawEMGAttentionLSTM, self).__init__()
...
print(f"\nRaw EMG Attention-LSTM Model parameters: {total_params:,}")
...
# 学習実行
print("\n" + "="*60)
print("Training Raw EMG Attention-LSTM started...")
print("="*60)
...
for epoch in epoch_pbar:
    ...
    if test_loss < best_test_loss:
        ...
        torch.save(model.state_dict(), 'best_model_raw_emg_attention_lstm.pth')
        marker = " *"
    else:
        marker = ""
...
plt.xlabel('Epoch')
plt.ylabel('MSE Loss')
plt.title('Raw EMG Attention-LSTM Training and Test Loss (window=64)')
plt.legend()
plt.grid(True)
plt.savefig('raw_emg_attention_lstm_training_loss.png', dpi=150)
plt.close()
print("Saved: raw_emg_attention_lstm_training_loss.png")

# 予測評価
model.load_state_dict(torch.load('best_model_raw_emg_attention_lstm.pth'))
model.eval()
...
print("\n" + "="*60)
print("Raw EMG Attention-LSTM Correlation Coefficients per Joint:")
print("="*60)
...
for i, ax in enumerate(axes.flat):
    ...
    ax.grid(True)
plt.tight_layout()
plt.savefig('raw_emg_attention_lstm_prediction_comparison.png', dpi=150)
plt.close()
print("Saved: raw_emg_attention_lstm_prediction_comparison.png")
...
plt.title('Raw EMG Attention-LSTM Prediction Correlation per Joint (window=64)')
plt.xticks(range(1, 23))
plt.legend()
plt.grid(True, axis='y')
plt.savefig('raw_emg_attention_lstm_correlation_per_joint.png', dpi=150)
plt.close()
print("Saved: raw_emg_attention_lstm_correlation_per_joint.png")

# 結果をJSONに保存
results = {
    'model': 'Raw-EMG-Attention-LSTM',
...
with open('raw_emg_attention_lstm_results.json', 'w') as f:
    json.dump(results, f, indent=2)
print("Saved: raw_emg_attention_lstm_results.json")

# =====================
# MelSpectrogram版との比較
# =====================
print("\n" + "="*60)
print("COMPARISON: Raw EMG vs MelSpectrogram (same window=64)")
print("="*60)

try:
    with open('melspec_attention_lstm_results.json', 'r') as f:
        melspec_results = json.load(f)

    print(f"{'Metric':<20} {'Raw EMG':>15} {'MelSpectrogram':>15} {'Difference':>15}")
    print("-"*65)
    print(f"{'Test Loss':<20} {best_test_loss:>15.4f} {melspec_results['best_test_loss']:>15.4f} {best_test_loss - melspec_results['best_test_loss']:>+15.4f}")
    print(f"{'Mean Correlation':<20} {np.mean(correlations):>15.4f} {melspec_results['mean_correlation']:>15.4f} {np.mean(correlations) - melspec_results['mean_correlation']:>+15.4f}")
    print(f"{'Max Correlation':<20} {np.max(correlations):>15.4f} {melspec_results['max_correlation']:>15.4f} {np.max(correlations) - melspec_results['max_correlation']:>+15.4f}")
    print(f"{'Min Correlation':<20} {np.min(correlations):>15.4f} {melspec_results['min_correlation']:>15.4f} {np.min(correlations) - melspec_results['min_correlation']:>+15.4f}")
    print(f"{'Parameters':<20} {total_params:>15,} {melspec_results['total_parameters']:>15,}")
except Exception as e:
    print(f"Could not load MelSpectrogram results: {e}")

print("\n" + "="*60)
print("Raw EMG Attention-LSTM RESULTS SUMMARY")
print("="*60)

"""
Self-Attention LSTM モデルでの学習・評価
シンプルなSingle-Head Self-Attentionを使用
"""
...
print("="*60)
print("Self-Attention LSTM Model Training (Single-Head)")
print("="*60)
...
print(f"  attention_type: Single-Head Self-Attention")
print()
...
# =====================
# Self-Attention LSTMモデル定義
# =====================
class EMGtoJointSelfAttentionLSTM(nn.Module):
    def __init__(self, input_size=16, hidden_size=256, num_layers=2, fc_size=64,
                 dropout=0.2, output_size=22, attention_dim=64):
        super(EMGtoJointSelfAttentionLSTM, self).__init__()
...
model = EMGtoJointSelfAttentionLSTM(
...
print(f"\nSelf-Attention LSTM Model parameters: {total_params:,}")
...
# 学習実行
print("\n" + "="*60)
print("Training Self-Attention LSTM started...")
print("="*60)
...
for epoch in epoch_pbar:
    ...
    if test_loss < best_test_loss:
        ...
        torch.save(model.state_dict(), 'best_model_self_attention_lstm.pth')
        marker = " *"
    else:
        marker = ""
...
plt.xlabel('Epoch')
plt.ylabel('MSE Loss')
plt.title('Self-Attention LSTM Training and Test Loss')
plt.legend()
plt.grid(True)
plt.savefig('self_attention_lstm_training_loss.png', dpi=150)
plt.close()
print("Saved: self_attention_lstm_training_loss.png")

# 予測評価
model.load_state_dict(torch.load('best_model_self_attention_lstm.pth'))
model.eval()
...
print("\n" + "="*60)
print("Self-Attention LSTM Correlation Coefficients per Joint:")
print("="*60)
...
for i, ax in enumerate(axes.flat):
    ...
    ax.grid(True)
plt.tight_layout()
plt.savefig('self_attention_lstm_prediction_comparison.png', dpi=150)
plt.close()
print("Saved: self_attention_lstm_prediction_comparison.png")
...
plt.title('Self-Attention LSTM Prediction Correlation per Joint')
plt.xticks(range(1, 23))
plt.legend()
plt.grid(True, axis='y')
plt.savefig('self_attention_lstm_correlation_per_joint.png', dpi=150)
plt.close()
print("Saved: self_attention_lstm_correlation_per_joint.png")
...
results = {
    ...
    'hyperparameters': {
        ...
        'epochs': EPOCHS,
        'attention_dim': ATTENTION_DIM,
        'attention_type': 'Single-Head Self-Attention'
    }
}
with open('self_attention_lstm_results.json', 'w') as f:
    json.dump(results, f, indent=2)
print("Saved: self_attention_lstm_results.json")
...
models_data.append(('Self-Attn-LSTM', best_test_loss, float(np.mean(correlations)), total_params))

print(f"{'Model':<22} {'Test Loss':>12} {'Mean Corr':>12} {'Parameters':>15}")
print("-"*65)
for name, loss, corr, params in models_data:
    print(f"{name:<22} {loss:>12.4f} {corr:>12.4f} {params:>15,}")

print("\n" + "="*60)
print("Self-Attention LSTM RESULTS SUMMARY")
print("="*60)

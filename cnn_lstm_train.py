...
class EMGtoJointCNNLSTM(nn.Module):
    def __init__(self, input_size=16, hidden_size=256, num_layers=2, fc_size=64, dropout=0.2, output_size=22):
        ...
        # CNN部分: 時系列の特徴を抽出
        # 入力: (batch, seq_len, channels) -> (batch, channels, seq_len) for Conv1d
    ...
    def forward(self, x):
        ...
        # Conv1dは (batch, channels, seq_len) を期待
...
print(f"\nCNN-LSTM Model parameters: {total_params:,}")
...
# 学習実行
print("\n" + "="*60)
print("Training CNN-LSTM started...")
print("="*60)
...
plt.xlabel('Epoch')
plt.ylabel('MSE Loss')
plt.title('CNN-LSTM Training and Test Loss')
plt.legend()
plt.grid(True)
plt.savefig('cnn_lstm_training_loss.png', dpi=150)
plt.close()
print("Saved: cnn_lstm_training_loss.png")
...
print("\n" + "="*60)
print("CNN-LSTM Correlation Coefficients per Joint:")
print("="*60)
...
for i, ax in enumerate(axes.flat):
    ...
    ax.grid(True)
plt.tight_layout()
plt.savefig('cnn_lstm_prediction_comparison.png', dpi=150)
plt.close()
print("Saved: cnn_lstm_prediction_comparison.png")
...
plt.title('CNN-LSTM Prediction Correlation per Joint')
plt.xticks(range(1, 23))
plt.legend()
plt.grid(True, axis='y')
plt.savefig('cnn_lstm_correlation_per_joint.png', dpi=150)
plt.close()
print("Saved: cnn_lstm_correlation_per_joint.png")
...
try:
    ...
    print(f"{'Metric':<25} {'LSTM':>12} {'CNN-LSTM':>12} {'Diff':>12}")
    print("-"*60)
    print(f"{'Best Test Loss':<25} {lstm_results['best_test_loss']:>12.4f} {best_test_loss:>12.4f} {best_test_loss - lstm_results['best_test_loss']:>+12.4f}")
    print(f"{'Mean Correlation':<25} {lstm_results['mean_correlation']:>12.4f} {np.mean(correlations):>12.4f} {np.mean(correlations) - lstm_results['mean_correlation']:>+12.4f}")
    print(f"{'Max Correlation':<25} {lstm_results['max_correlation']:>12.4f} {np.max(correlations):>12.4f} {np.max(correlations) - lstm_results['max_correlation']:>+12.4f}")
    print(f"{'Parameters':<25} {826166:>12,} {total_params:>12,}")
except:
    print("Could not load LSTM results for comparison")

print("\n" + "="*60)
print("CNN-LSTM RESULTS SUMMARY")
print("="*60)

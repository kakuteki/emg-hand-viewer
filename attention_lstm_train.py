"""
Attention-LSTM モデルでの学習・評価
LSTMの前にSelf-Attentionを配置して比較
"""
...
print("="*60)
print("Attention-LSTM Model Training")
print("="*60)
...
# Attention-LSTMモデル定義
# =====================
class EMGtoJointAttentionLSTM(nn.Module):
    def __init__(self, input_size=16, hidden_size=256, num_layers=2, fc_size=64,
        ...
        super(EMGtoJointAttentionLSTM, self).__init__()
...
print(f"\nAttention-LSTM Model parameters: {total_params:,}")
...
# 学習実行
print("\n" + "="*60)
print("Training Attention-LSTM started...")
print("="*60)
...
for epoch in epoch_pbar:
    ...
    if test_loss < best_test_loss:
        ...
        torch.save(model.state_dict(), 'best_model_attention_lstm.pth')
        marker = " *"
    else:
        marker = ""
...
plt.xlabel('Epoch')
plt.ylabel('MSE Loss')
plt.title('Attention-LSTM Training and Test Loss')
plt.legend()
plt.grid(True)
plt.savefig('attention_lstm_training_loss.png', dpi=150)
plt.close()
print("Saved: attention_lstm_training_loss.png")

# 予測評価
model.load_state_dict(torch.load('best_model_attention_lstm.pth'))
model.eval()
...
print("\n" + "="*60)
print("Attention-LSTM Correlation Coefficients per Joint:")
print("="*60)
...
for i, ax in enumerate(axes.flat):
    ...
    ax.grid(True)
plt.tight_layout()
plt.savefig('attention_lstm_prediction_comparison.png', dpi=150)
plt.close()
print("Saved: attention_lstm_prediction_comparison.png")
...
plt.title('Attention-LSTM Prediction Correlation per Joint')
plt.xticks(range(1, 23))
plt.legend()
plt.grid(True, axis='y')
plt.savefig('attention_lstm_correlation_per_joint.png', dpi=150)
plt.close()
print("Saved: attention_lstm_correlation_per_joint.png")
...
print("Saved: attention_lstm_results.json")
...
models_data.append(('Attention-LSTM', best_test_loss, float(np.mean(correlations)), total_params))
...
print("\n" + "="*60)
print("Attention-LSTM RESULTS SUMMARY")
print("="*60)
